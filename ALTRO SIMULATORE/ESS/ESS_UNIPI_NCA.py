...
class ESS_UNIPI_NCA(ESS):
    ...
    def __init__(self, model, p_S_max, a, b, B, eta_inverter, C_n, Ns_batt, Np_batt, SoC_0, SoC_min, SoC_max, T):
        ...
        self.T = T
        self.V_n = 3.65 # nominal value for NCA cells
        self.C_n = C_n
        self.eta_inverter = eta_inverter

        self.Q_n = self.C_n * self.V_n * self.Ns_batt * self.Np_batt / 1000
        self.Q = self.Q_n * self.SoE_0
        self.parameters = sio.loadmat(os.path.join("ESS_package_UNIPI", "parameters_cell_NCA.mat"))["parameters_cell_NCA"]
        self.SoC_interpol = np.linspace(0, 1, len(self.parameters))
        self.T_interpol = np.array([20, 40])
        self.R0_table = self.parameters[:, 1:3] * (self.Ns_batt / self.Np_batt) *3/self.C_n # scaled from base-paramters of a 3 Ah NCA cell
